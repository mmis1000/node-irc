...
Client.prototype._cauculateByteLength = function(str) {
    var totalLength = 0,
        temp = str.split(''),
        code = null;
...
Client.prototype._splitLongLines = function(words, maxLength, destination) {
    ...
    //console.log('debug0: ', words, maxLength, destination);
    var byteLength = this._cauculateByteLength(words);
    if (byteLength <= maxLength) {
        destination.push(words);
        return destination;
    }
    var minSplitRatio = 0;
    var temp = words.split('').map(this._cauculateByteLength);
    var cutPos = null;
    ...
    for (var i = 0; i < temp.length; i++) {
        // check if it exceed the maxLength
        if (currentByteLength + temp[i] > maxLength) {
            if (cutPos && currentByteLength > maxLength * minSplitRatio) {
                destination.push(words.slice(segmentStart, cutPos));
                segmentStart = cutPos;
            } else {
                destination.push(words.slice(segmentStart, i));
                segmentStart = i;
            }
            // find next segmaent
            i = segmentStart - 1 ;
        ...
        currentByteLength += temp[i];
        if (i === temp.length - 1) {
            destination.push(words.slice(segmentStart, temp.length));
            break;
        }
        if (seperators.indexOf(words[i - 1]) >= 0) {
            cutPos = i;
        }
    }
    return destination;
